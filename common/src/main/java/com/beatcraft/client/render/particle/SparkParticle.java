...
public class SparkParticle implements Particle {
    ...
    public void update(float deltaTime, BufferBuilder buffer, Vector3f cameraPos) {
        ...
        for (Vector3f[] vert : vertices) {
            buffer.addVertex(vert[0].x, vert[0].y, vert[0].z).setColor(col);
            buffer.addVertex(vert[1].x, vert[1].y, vert[1].z).setColor(col);
            buffer.addVertex(vert[2].x, vert[2].y, vert[2].z).setColor(col);
        }
