...
void main() {
    ...
    float distortionStrength = 0.01 / screenUV.z;
    float timeFactor = Time * 1.25;
    vec3 noiseInput = worldPos + vec3(timeFactor);
    vec2 distortion = vec2(
        cnoise(noiseInput + vec3(23.1, 0.0, 0.0)),
    ...
    fragColor = blendColors(color, vec4(vertexColor.rgb, 0.5));
}