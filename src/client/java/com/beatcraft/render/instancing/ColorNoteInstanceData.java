...
public class ColorNoteInstanceData implements InstancedMesh.InstanceData {
    ...
    public ColorNoteInstanceData(Matrix4f transform, Color color, float dissolve) {
    ...
    private static final int DISSOLVE_LOCATION = 8;
    ...
    public void init() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            GL20.glVertexAttribPointer(location, 4, GL11.GL_FLOAT, false,
                MATRIX4F_SIZE_BYTES + COLOR_SIZE_BYTES + FLOAT_SIZE_BYTES, i * 4 * FLOAT_SIZE_BYTES);
        ...
        GL20.glVertexAttribPointer(COLOR_LOCATION, 4, GL11.GL_FLOAT, false,
            MATRIX4F_SIZE_BYTES + COLOR_SIZE_BYTES + FLOAT_SIZE_BYTES, MATRIX4F_SIZE_BYTES);
        GL20.glEnableVertexAttribArray(COLOR_LOCATION);
        ARBInstancedArrays.glVertexAttribDivisorARB(COLOR_LOCATION, 1);

        GL20.glVertexAttribPointer(DISSOLVE_LOCATION, 1, GL11.GL_FLOAT, false,
            MATRIX4F_SIZE_BYTES + COLOR_SIZE_BYTES + FLOAT_SIZE_BYTES, MATRIX4F_SIZE_BYTES + COLOR_SIZE_BYTES);
        GL20.glEnableVertexAttribArray(DISSOLVE_LOCATION);
        ARBInstancedArrays.glVertexAttribDivisorARB(DISSOLVE_LOCATION, 1);
